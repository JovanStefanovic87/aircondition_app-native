import { DatabaseVersionType, DeviceStateByInspection, DeviceStateComponent, DeviceStateComponentsForInspection, DeviceStateValueDetails, DeviceType, Inspection, InspectionDeviceComponentUpdate, InspectionType, TitleComponent, User } from "../../types";
...
export const getInspectionDeviceStateByGroupType = async (inspectionId: string): Promise<DeviceStateByInspection[]> => {
    const query = `
        SELECT dsc.*, gt.name as groupTypeName, tc.name as titleComponentName, ids.value, ids.note FROM DeviceStateComponent dsc
...
export const getDeviceStateValues = async (inspectionId: string): Promise<DeviceStateValueDetails[]> => {
    const query = `
...
export const getInspectionDeviceStateDetails = async (inspectionId: string): Promise<DeviceStateComponentsForInspection[]> => {
    const inspectionDeviceStateByGroupType = await getInspectionDeviceStateByGroupType(inspectionId);
    const deviceStateValues = await getDeviceStateValues(inspectionId);
    const uniqueGroupTypeNames = [...new Set(inspectionDeviceStateByGroupType.map(item => item.groupTypeName))];
    const finalResult: DeviceStateComponentsForInspection[] = [];

    for (const groupTypeName of uniqueGroupTypeNames) {
        const groupTypeItems = inspectionDeviceStateByGroupType.filter(item => item.groupTypeName === groupTypeName);
        ...
        for (const title of titleNames) {
            ...
            const titleComponent = {
                name: title,
                deviceStateComponents: titleItems.filter(titleItem => titleItem.titleComponentName === title).map(titleItem => {
                    const deviceStateComponent = {
                        id: titleItem.deviceStateId,
                        name: titleItem.name,
                        ...
                        deviceStateValues: deviceStateValues.filter(value => value.deviceStateComponentId === titleItem.deviceStateId)
                    };
